        ...
        if closestVehicle == 0 then
            if Framework == 'esx' then
                Core.Game.SpawnVehicle(vehicle, spawnCoords, spawnCoords.w, function(vehicle)
                    ...
                    if not isTest then
                        TriggerServerEvent('ux-starterpack:ClaimVehicle', vehicleData)
                    end
                end)
            else
...
local function InitializeScenario()
    ...
    for _, ped in pairs(Peds) do
        ...
        if #(playerCoords - pedCoords) < 3 then
            ...
            TriggerServerEvent("ux-starterpack:ClaimStarterpack")
            ...
            options[#options + 1] = {
                ...
                onSelect = function()
                    ...
                    if alert == "confirm" or not Config.EnableAlertDialog then
                        lib.callback('ux-starterpack:CheckPlayer', false, function(data)
                            if data then
...
if Config.UseCommand then
    RegisterCommand(Config.Command, function(source, args, raw)
        ...
        if alert == "confirm" or not Config.EnableAlertDialog then
            lib.callback('ux-starterpack:CheckPlayer', source, function(data)
                if data then
                    if lib.progressCircle({
                            duration = 3000,
                            position = 'bottom',
